...
kartka Zlozona(kartka k, wektor v) {
	kartka rez;
	if (k.form == Rec) {
		rez = Prostokat(k.k.r.a, k.k.r.b);
	} else {
		rez = Kolo(k.k.c.s, k.k.c.r);
	}
...
int czyPrzebija(kartka k, punkt p) {
	if (k.form == Rec) {
    return fmin(p.x - k.k.r.a.x, p.y - k.k.r.a.y) > -EPS && fmin(k.k.r.b.x - p.x, k.k.r.b.y - p.y) > -EPS;
  } else { 
		return fkwadrat(p.x - k.k.c.s.x) + fkwadrat(p.y - k.k.c.s.y) < fkwadrat(k.k.c.r) + EPS;
  }
}
